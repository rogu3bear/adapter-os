...
describe('LoginPage', () => {
  ...
  describe('rendering', () => {
    ...
    it('renders AdapterOS branding', () => {
      ...
      expect(screen.getByText(/sign in to the control plane/i)).toBeInTheDocument();
    });
  ...
  describe('TOTP field toggle', () => {
    it('shows TOTP field when "Add TOTP code" button is clicked', async () => {
      ...
      expect(screen.getByRole('button', { name: /add totp code/i })).toBeInTheDocument();

      // Click the "Add TOTP code" button
      const showTotpButton = screen.getByRole('button', { name: /add totp code/i });
