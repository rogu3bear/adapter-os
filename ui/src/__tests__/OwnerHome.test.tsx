...
const mockSystemOverview: SystemOverview = {
  ...
  services: [
    { name: 'API Server', status: 'healthy', last_check: Date.now() },
    { name: 'Worker Pool', status: 'healthy', last_check: Date.now() },
...
vi.mock('@/api/services', () => ({
  ...
  apiClient: {
    ...
    getBaseModelStatus: (...args: unknown[]) => Promise.resolve({
      model_id: null,
      model_name: null,
      status: 'no-model',
      backend: null,
...
describe('OwnerHomePage', () => {
  ...
  describe('Initial Rendering', () => {
    it('renders the owner home page with correct title', async () => {
      ...
      expect(screen.getByText(/Owner Home \(Legacy\)/i)).toBeTruthy();
    });
  ...
  describe('Data Display', () => {
    ...
    it('displays tenant count', async () => {
      ...
      // Wait for tenants to load first
      await waitFor(() => {
  ...
  describe('Refresh Functionality', () => {
    it('refreshes all data when refresh button is clicked', async () => {
      ...
      const initialSystemCalls = mockGetSystemOverview.mock.calls.length;
      const initialTenantCalls = mockListTenants.mock.calls.length;
      const initialAdapterCalls = mockListAdapters.mock.calls.length;
      const initialStackCalls = mockListAdapterStacks.mock.calls.length;
      const initialModelCalls = mockListModels.mock.calls.length;
      ...
      await waitFor(() => {
        expect(mockGetSystemOverview.mock.calls.length).toBeGreaterThan(initialSystemCalls);
        expect(mockListTenants.mock.calls.length).toBeGreaterThan(initialTenantCalls);
        expect(mockListAdapters.mock.calls.length).toBeGreaterThan(initialAdapterCalls);
        expect(mockListAdapterStacks.mock.calls.length).toBeGreaterThan(initialStackCalls);
        expect(mockListModels.mock.calls.length).toBeGreaterThan(initialModelCalls);
      }, { timeout: 3000 });
    });
  ...
  describe('Edge Cases', () => {
    ...
    it('handles empty adapter list', async () => {
      ...
      // Wait for adapters to load first
      await waitFor(() => {
        expect(mockListAdapters).toHaveBeenCalled();
      }, { timeout: 3000 });
