...
import { Adapter, InferRequest, InferResponse, BatchInferResponse } from '@/api/types';
...
export default function MLEngineerInferenceTest() {
  ...
  const runBatchInference = useCallback(async () => {
    ...
    try {
      // BatchInferItemRequest extends InferRequest with an id field
      const requests: components['schemas']['BatchInferItemRequest'][] = validPrompts.map((prompt, idx) => ({
        id: `req-${idx}`,
        prompt,
      ...
      const batchRequest: components['schemas']['BatchInferRequest'] = {
        requests,
      };

      // If verifying determinism, run multiple times and compare
      if (verifyDeterminism) {
        const allRuns: components['schemas']['BatchInferItemResponse'][][] = [];
        ...
        const batchResults: BatchResult[] = validPrompts.map((prompt, idx) => {
          const responses = allRuns.map(run => run[idx]?.response).filter((r): r is InferResponse => r != null);
          const firstResponse = responses[0];

          // Check if all responses match
          const allMatch = responses.length === determinismRuns && responses.every(r =>
        ...
        const response = await apiClient.batchInfer(batchRequest);
        const responseItems = response.responses || [];

        const batchResults: BatchResult[] = validPrompts.map((prompt, idx) => ({
          prompt,
          response: responseItems[idx]?.response || null,
        }));

        setResults(batchResults);
        setBatchResponse(response);
        toast.success(`Batch inference completed: ${responseItems.length} responses`);
      }
