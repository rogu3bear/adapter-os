          ...
          <Tabs defaultValue="cpu" className="space-y-4">
            ...
            <TabsContent value="cpu" className="space-y-2">
              <div className="text-sm text-muted-foreground">
                CPU usage over the last {metrics.length} data points
              </div>
            ...
            <TabsContent value="memory" className="space-y-2">
              <div className="text-sm text-muted-foreground">
                Memory usage over the last {metrics.length} data points
              </div>
              <div className="h-64 w-full border rounded-md p-4">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={metrics}>
            ...
            <TabsContent value="gpu" className="space-y-2">
              <div className="text-sm text-muted-foreground">
                GPU utilization and memory over the last {metrics.length} data points
              </div>
              <div className="h-64 w-full border rounded-md p-4">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={metrics}>
                    ...
                    <ReTooltip
                      ...
                      formatter={(value: number, name: string) => [
                        `${value.toFixed(1)}%`,
                        name === 'gpu.utilization' ? 'GPU Utilization' : 'GPU Memory Usage'
                      ]}
                    />
                    <Line
                      type="monotone"
            ...
            <TabsContent value="disk" className="space-y-2">
              <div className="text-sm text-muted-foreground">
                Disk I/O activity over the last {metrics.length} data points
              </div>
              <div className="h-64 w-full border rounded-md p-4">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={metrics}>
                    ...
                    <ReTooltip
                      ...
                      formatter={(value: number, name: string) => [
                        ...
                        name === 'disk.io_read' ? 'Read' : 'Write'
                      ]}
                    />
                    <Line
                      type="monotone"
                      dataKey="disk.io_read"
                      stroke="#06b6d4"
                      strokeWidth={2}
                      dot={false}
                      name="Read"
