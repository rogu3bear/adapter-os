  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      if (!shouldAllowConnection()) {
        ...
        setError(`Service unavailable (circuit open). Retry in ${Math.ceil(remainingMs / 1000)}s`);
      ...
      eventSource.onerror = (event) => {
        ...
        if (reconnectAttemptsRef.current < MAX_RECONNECT_ATTEMPTS) {
          ...
          setError(`${errorMessage}. Reconnecting in ${backoffMs / 1000}s (attempt ${reconnectAttemptsRef.current}/${MAX_RECONNECT_ATTEMPTS})`);
          ...
          setError(`SSE connection failed after ${MAX_RECONNECT_ATTEMPTS} attempts: ${errorMessage}`);
