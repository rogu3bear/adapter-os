...
export function useSessionTelemetry(options: UseSessionTelemetryOptions = {}): UseSessionTelemetryResult {
  ...
  const {
    ...
    queryKey: ['session-telemetry', 'sessions', tenantId, page, pageSize, filterRequestId, sourceType] as const,
    queryFn: async (): Promise<TransformedRoutingDecision[]> =>
  ...
  const sessions = useMemo(() => uniqueSessions(decisions as TransformedRoutingDecision[] | undefined), [decisions]);
  ...
  const {
    ...
    queryKey: ['session-telemetry', 'steps', selectedRequestId] as const,
    queryFn: async (): Promise<SessionRouterViewResponse> => {
  ...
  const steps: SessionStep[] = useMemo(
    () => (sessionRouterView as SessionRouterViewResponse | undefined)?.steps ?? [],
    [sessionRouterView]
  );
  ...
  const {
    ...
    queryKey: ['session-telemetry', 'adapter-map', (sessionRouterView as SessionRouterViewResponse | undefined)?.stack_id] as const,
    queryFn: async () => {
      const map = new Map<number, { id: string; name: string }>();
      const stackId = (sessionRouterView as SessionRouterViewResponse | undefined)?.stack_id;
    ...
    enabled: !!(sessionRouterView as SessionRouterViewResponse | undefined)?.stack_id,
    staleTime: 60_000,
  });
  ...
  const {
    ...
    queryKey: ['session-telemetry', 'metrics-tps', selectedRequestId, timeWindow?.startMs, timeWindow?.endMs] as const,
    queryFn: async (): Promise<MetricsSeriesResponse[]> => {
  ...
  const { data: latencySeries } = useQuery({
    queryKey: ['session-telemetry', 'metrics-latency', selectedRequestId, timeWindow?.startMs, timeWindow?.endMs] as const,
    queryFn: async (): Promise<MetricsSeriesResponse[]> => {
      if (!timeWindow) return [];
      return apiClient.getMetricsSeries({
  ...
  const tokensPerSecond = useMemo(() => flattenMetrics(tokensPerSecondSeries as MetricsSeriesResponse[] | undefined), [tokensPerSecondSeries]);
  const latencyP50 = useMemo(() => flattenMetrics(latencySeries as MetricsSeriesResponse[] | undefined), [latencySeries]);
