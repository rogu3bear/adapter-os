  ...
  const start = useCallback(async (config: StreamingInferenceConfig) => {
    ...
    try {
        ...
        for (const event of events) {
          ...
          try {
            ...
            for (const choice of chunk.choices) {
              ...
              if (content && isMountedRef.current && !abortControllerRef.current?.signal.aborted) {
                // Accumulate text
              ...
              // Check finish reason
              if (choice.finish_reason && isMountedRef.current && !abortControllerRef.current?.signal.aborted) {
      ...
      logger.info('Streaming inference finished', {
        ...
        tokensPerSec: Date.now() - startTimeRef.current > 0 ? (tokenCountRef.current / (Date.now() - startTimeRef.current)) * 1000 : 0,
      });
