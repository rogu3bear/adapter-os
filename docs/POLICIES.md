...
3. [The 20 Canonical Policy Packs](#the-20-canonical-policy-packs)
...
AdapterOS implements a comprehensive policy enforcement system with 20 canonical policy packs that govern all aspects of system behavior. Policy enforcement happens at multiple layers throughout the request lifecycle, ensuring security, performance, and operational compliance.
...
All policy packs are registered via `initialize_policy_packs()` in `crates/adapteros-policy/src/policy_packs.rs`.
...
**Rules:**
- Memory headroom: always keep 20% free
...
**Purpose:** Complete policy pack example

This is a comprehensive policy pack that serves as a reference implementation, demonstrating all policy features including validation, enforcement levels, and remediation guidance.
...
| `numeric-units` | Numeric Units Policy | Medium |
| `rag-index` | RAG Index Policy | High |
...
| `build-release` | Build Release Policy | Medium |
| `compliance` | Compliance Policy | Medium |
| `incident` | Incident Policy | High |
| `llm-output` | LLM Output Policy | Medium |
| `adapter-lifecycle` | Adapter Lifecycle Policy | Medium |
| `full-pack` | Full Pack Policy | Low |
...
- [CLAUDE.md](../CLAUDE.md) - Developer guide and policy implementation
