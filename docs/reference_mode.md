# Reference Mode (Environment Overlay)

`configs/reference.env` is a small, sourceable environment overlay for running repeatable local reference runs. It pins ports, a dedicated SQLite DB file, logging verbosity, and a few common timeouts without changing runtime logic or code.
...
2. Load the reference overlay into your shell:

```bash
set -a
source configs/reference.env
set +a
```
...
This repo includes a `.envrc` that sources `.env` and `.env.local`. To apply reference settings automatically:
...
- Stop the server and delete the reference DB file: `rm -f var/aos-reference.sqlite3`
