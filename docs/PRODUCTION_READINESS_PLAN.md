...
**Status:** 65/100 (Security Audit: 85/100) — Phase 1 fixes completed
**Target:** 95/100
...
`agent_log()` is not present in `crates/adapteros-server/src/main.rs`.
...
CORS is now restricted in the supervisor and UDS API paths using explicit allowlists (`ALLOWED_ORIGINS` in production, localhost defaults in dev).
...
`skip_pf_check` and `skip_drift_check` are gated to debug builds via `cfg_attr` and are skipped in release builds.
...
Keypairs load from PEM or `var/keys/supervisor_signing.key` with self-healing and production warnings when regeneration occurs.
...
**Status:** ✅ Implemented (installed in `routes.rs`, covered by security header tests)
...
### Security
- [x] Remove `agent_log()` debug function
- [x] Fix permissive CORS in service supervisor
- [x] Restrict `--skip-pf-check` and `--skip-drift-check` flags
