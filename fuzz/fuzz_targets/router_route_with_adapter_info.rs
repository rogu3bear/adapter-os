...
fuzz_target!(|data: &[u8]| {
    ...
    let decision_a = router_a.route_with_adapter_info(&features, &priors, &adapters, &policy_mask);
    let decision_b = router_b.route_with_adapter_info(&features, &priors, &adapters, &policy_mask);
    ...
    assert_eq!(decision_a.policy_mask_digest, decision_b.policy_mask_digest);
