    ...
    pub(crate) fn run(&self) {
        let mut borrow = match self.inner.try_borrow_mut() {
            ...
            Err(_) => {
                ...
                // run() call (re-entrancy through V8 microtask checkpoints).
                // The re-entrancy guard in run_all should prevent reaching here,
                // but if it does, return gracefully instead of panicking.
                // is_queued is still true (not yet cleared), so the waker
                // mechanism will re-schedule this task for a future poll.
                return;
            }
        };
