...
# Exceptions: README.md, AGENTS.md, files in archive/
# -----------------------------------------------------------------------------
...
while IFS= read -r -d '' file; do
    ...
    # Skip README.md, AGENTS.md, CHANGELOG.md, LICENSE.md, CONTRIBUTING.md
...
if git rev-parse --git-dir > /dev/null 2>&1; then
    ...
    if [[ -n "$staged_root" ]]; then
        # Root docs should be mentioned in main README or AGENTS.md References section
        readme="$PROJECT_ROOT/README.md"
        claude="$PROJECT_ROOT/AGENTS.md"
        for doc in $staged_root; do
            ...
            if [[ "$found" == "false" ]]; then
                index_issues+="  - $doc not referenced in README.md or AGENTS.md\n"
            fi
        done
    fi
fi
