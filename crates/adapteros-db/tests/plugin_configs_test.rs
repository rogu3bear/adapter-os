...
async fn test_plugin_config_crud() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(config.config_json, Some(r#"{"scan_interval": 300}"#.to_string()));

    // Test 3: Update plugin config
    db.upsert_plugin_config("code-intelligence", false, Some(r#"{"scan_interval": 600}"#))
        .await?;
    ...
    let config = config.unwrap();
    assert!(!config.enabled);
    assert_eq!(config.config_json, Some(r#"{"scan_interval": 600}"#.to_string()));
...
async fn test_tenant_plugin_enables() -> Result<(), Box<dyn std::error::Error>> {
    ...
    db.upsert_plugin_config("code-intelligence", false, None).await?;

    // Test 1: Enable plugin for specific tenant
    db.enable_plugin_for_tenant("code-intelligence", "tenant-123", Some(r#"{"custom": true}"#))
        .await?;

    // Test 2: Check if plugin is enabled for tenant (should override global setting)
    let is_enabled = db.is_plugin_enabled_for_tenant("code-intelligence", "tenant-123").await?;
    assert!(is_enabled);

    // Test 3: Check if plugin is enabled for different tenant (should use global setting)
    let is_enabled = db.is_plugin_enabled_for_tenant("code-intelligence", "tenant-456").await?;
    assert!(!is_enabled);

    // Test 4: Disable plugin for tenant
    db.disable_plugin_for_tenant("code-intelligence", "tenant-123").await?;
    let is_enabled = db.is_plugin_enabled_for_tenant("code-intelligence", "tenant-123").await?;
    assert!(!is_enabled);

    // Test 5: List tenant plugin enables
    db.enable_plugin_for_tenant("telemetry", "tenant-123", None).await?;
...
async fn test_plugin_global_vs_tenant_precedence() -> Result<(), Box<dyn std::error::Error>> {
    ...
    db.upsert_plugin_config("code-intelligence", true, None).await?;

    // Test 1: Tenant without override uses global setting
    let is_enabled = db.is_plugin_enabled_for_tenant("code-intelligence", "tenant-999").await?;
    assert!(is_enabled);

    // Test 2: Tenant with override uses tenant setting (disabled)
    db.disable_plugin_for_tenant("code-intelligence", "tenant-999").await?;
    let is_enabled = db.is_plugin_enabled_for_tenant("code-intelligence", "tenant-999").await?;
    assert!(!is_enabled);

    // Test 3: Tenant with override uses tenant setting (enabled)
    db.enable_plugin_for_tenant("code-intelligence", "tenant-888", None).await?;
    let is_enabled = db.is_plugin_enabled_for_tenant("code-intelligence", "tenant-888").await?;
    assert!(is_enabled);
