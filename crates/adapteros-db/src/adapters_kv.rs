...
impl AdapterKvRepository {
    ...
    fn sort_adapters_deterministically(adapters: &mut [Adapter]) {
...
impl From<Adapter> for AdapterKv {
    fn from(adapter: Adapter) -> Self {
        AdapterKv {
            ...
            metadata_json: adapter.metadata_json,
    ...
    fn test_deterministic_sorting() {
        let mut adapters = vec![
            Adapter {
                ...
                commit_sha: None,
                intent: None,
                current_state: "unloaded".into(),
                pinned: 0,
                memory_bytes: 0,
                last_activated: None,
                activation_count: 0,
                expires_at: None,
                load_state: "cold".into(),
                last_loaded_at: None,
                adapter_name: None,
                tenant_namespace: None,
                domain: None,
        ...
        AdapterKvRepository::sort_adapters_deterministically(&mut adapters);
        assert_eq!(adapters[0].id, "a");
        assert_eq!(adapters[1].id, "b");
    }
}
