...
// ============================================================================
// Dataset Scan Root Types
// ============================================================================

/// A scan root entry for a dataset, representing a source directory.
///
/// Each row captures provenance metadata for a directory that was scanned
/// during code ingestion to create training data.
#[derive(Debug, Clone, Serialize, Deserialize, sqlx::FromRow)]
...
pub struct CreateDatasetScanRootParams {
    /// Dataset this scan root belongs to (required)
    pub dataset_id: String,
    /// Dataset version ID (optional, can be set later)
    pub dataset_version_id: Option<String>,
    /// Session identifier for atomic operations
    pub session_id: Option<String>,
    /// Absolute or relative path to the scan root directory (required)
    pub path: String,
    /// Optional label describing this scan root's role
    pub label: Option<String>,
    /// Number of files processed from this scan root
    ...
    /// Tenant ID for multi-tenant isolation
    pub tenant_id: Option<String>,
    /// Who created this scan root entry
    pub created_by: Option<String>,
    /// Additional metadata as JSON
    pub metadata_json: Option<String>,
}
...
impl Db {
    ...
    /// Bulk insert dataset scan roots for efficiency.
    pub async fn bulk_insert_dataset_scan_roots(
        ...
        for params in roots {
            ...
            .execute(&mut *tx)
            .await
            .map_err(db_err("bulk insert dataset scan root"))?;
        ...
        tx.commit()
            .await
            .map_err(db_err("commit dataset scan root inserts"))?;

        Ok(count)
    }

    /// List scan roots for a dataset, ordered by ordinal.
    pub async fn list_dataset_scan_roots(&self, dataset_id: &str) -> Result<Vec<DatasetScanRoot>> {
        let query = format!(
            "SELECT {} FROM dataset_scan_roots WHERE dataset_id = ? ORDER BY ordinal ASC, created_at ASC",
    ...
    /// List scan roots for a dataset version, ordered by ordinal.
    pub async fn list_dataset_scan_roots_for_version(
        &self,
        dataset_version_id: &str,
    ) -> Result<Vec<DatasetScanRoot>> {
        let query = format!(
            "SELECT {} FROM dataset_scan_roots WHERE dataset_version_id = ? ORDER BY ordinal ASC, created_at ASC",
            DATASET_SCAN_ROOT_COLUMNS
        );
    ...
    /// List scan roots created within a specific ingestion session.
    pub async fn list_dataset_scan_roots_by_session(
        &self,
        session_id: &str,
    ) -> Result<Vec<DatasetScanRoot>> {
        let query = format!(
            "SELECT {} FROM dataset_scan_roots WHERE session_id = ? ORDER BY ordinal ASC, created_at ASC",
            DATASET_SCAN_ROOT_COLUMNS
        );
    ...
    /// Update dataset_version_id for all scan roots in a session.
    pub async fn update_dataset_scan_root_session_version(
        ...
        .execute(self.pool())
        .await
        .map_err(db_err("update dataset scan root session version"))?;

        Ok(result.rows_affected())
    }

    /// Delete scan roots for a specific ingestion session.
    pub async fn delete_dataset_scan_roots_by_session(&self, session_id: &str) -> Result<u64> {
        let result = sqlx::query("DELETE FROM dataset_scan_roots WHERE session_id = ?")
            ...
            .execute(self.pool())
            .await
            .map_err(db_err("delete dataset scan roots by session"))?;

        Ok(result.rows_affected())
    }

    /// Aggregate scan root statistics for a dataset.
