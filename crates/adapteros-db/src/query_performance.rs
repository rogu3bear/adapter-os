...
impl QueryPerformanceMonitor {
    ...
    pub fn with_config(
        ...
        Self {
            ...
            multi_tenant_config,
            concurrency_config,
            regression_thresholds: regression_thresholds.unwrap_or_else(|| RegressionThresholds {
...
pub async fn run_concurrency_performance_test(
    ...
    println!("Starting concurrency performance test:");
    println!("  Workers: {}", config.concurrent_workers);
    println!("  Duration: {}ms", config.test_duration_ms);
    println!(
        "  Queries/sec/worker: {}",
        config.queries_per_second_per_worker
    );
    println!("  Tenants: {}", tenant_ids.len());
    ...
    println!(
        "Concurrency test completed in {:.2}s",
        total_duration.as_secs_f64()
    );
    println!("Total queries executed: {}", all_metrics.len());
    ...
    if !all_metrics.is_empty() {
        ...
        println!("Concurrent query statistics:");
        println!("  Average latency: {:.2}ms", avg_latency / 1000.0);
        println!("  P95 latency: {:.2}ms", p95_latency as f64 / 1000.0);
        println!(
            "  Queries per second: {:.1}",
            all_metrics.len() as f64 / total_duration.as_secs_f64()
        );
    }
