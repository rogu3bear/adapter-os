...
impl MigrationStats {
    ...
    pub fn success_rate(&self) -> f64 {
        ...
        ((self.migrated + self.skipped) as f64 / self.total as f64) * 100.0
    }
}
...
impl Db {
    ...
    async fn migrate_single_adapter(
        ...
        if repo.get_adapter_kv(adapter_id).await?.is_some() {
            // Already in KV, skip migration
            return Ok(false);
        }
    ...
    async fn migrate_domain_adapters(
        ...
        for (idx, adapter) in adapters.into_iter().enumerate() {
            ...
            match self.migrate_single_adapter(&repo, adapter.clone()).await {
                ...
                Err(e) => {
                    stats.failed += 1;
                    stats.errors.push(format!("adapter {}: {}", adapter.id, e));
                }
            }
        }
        ...
        for (idx, record) in stacks.into_iter().enumerate() {
            ...
            match stack_record_to_kv(&record) {
                ...
                Err(e) => {
                    ...
                    stats
                        .errors
                        .push(format!("stack {} convert: {}", record.id, e));
                }
            }
        }
        ...
        for (idx, plan) in plans.into_iter().enumerate() {
            ...
            match plan_to_kv(&plan) {
                ...
                Err(e) => {
                    ...
                    stats
                        .errors
                        .push(format!("plan {} convert: {}", plan.id, e));
                }
            }
        }
    ...
    async fn migrate_domain_auth_sessions(
        ...
        for (idx, row) in rows.into_iter().enumerate() {
            ...
            if let Err(e) = repo.put_session(kv).await {
                ...
                stats
                    .errors
                    .push(format!("auth_session {}: {}", row.jti, e));
            } else {
                stats.migrated += 1;
            }
        }
    ...
    async fn migrate_domain_runtime_sessions(
        ...
        for (idx, row) in rows.into_iter().enumerate() {
            ...
            if let Err(e) = repo.put(&kv).await {
                ...
                stats
                    .errors
                    .push(format!("runtime_session {}: {}", kv.id, e));
            } else {
                stats.migrated += 1;
            }
        }
