...
pub struct SignaturesSchema {
    ...
    /// Ed25519 public key (base64-encoded PEM or raw key bytes)
...
impl MigrationVerifier {
    ...
    fn verify_signature(&self, file_hash: &str, signature_b64: &str) -> Result<()> {
        ...
        // Decode base64 to get PEM bytes or raw key bytes
        let public_key_bytes = general_purpose::STANDARD
            .decode(&self.signatures.public_key)
            .map_err(|e| AosError::Crypto(format!("Invalid public key base64 encoding: {}", e)))?;

        let public_key_bytes = match String::from_utf8(public_key_bytes.clone()) {
            Ok(public_key_pem) => Self::extract_ed25519_public_key_from_pem(&public_key_pem)?,
            Err(_) => {
                if public_key_bytes.len() != 32 {
                    return Err(AosError::Crypto(format!(
                        "Public key is not valid PEM and has unexpected length: {}",
                        public_key_bytes.len()
                    )));
                }
                let mut raw_key = [0u8; 32];
                raw_key.copy_from_slice(&public_key_bytes);
                raw_key
            }
        };
