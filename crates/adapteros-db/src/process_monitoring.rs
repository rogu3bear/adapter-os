...
impl PerformanceBaseline {
    ...
    pub async fn get(
        ...
        if let Some(row) = row {
            ...
            let expires_at_opt: Option<chrono::NaiveDateTime> = row.get("expires_at");
            ...
            Ok(Some(PerformanceBaseline {
                ...
                baseline_type: BaselineType::from_string(row.get("baseline_type")),
                ...
                is_active: row.get("is_active"),
                calculated_at: chrono::DateTime::parse_from_rfc3339(&calculated_at_str)
                    .map_err(|e| AosError::Database(format!("Invalid calculated_at: {}", e)))?
                    .with_timezone(&chrono::Utc),
                expires_at: expires_at_opt.map(|dt| dt.and_utc()),
            }))
        } else {
            Ok(None)
        }
