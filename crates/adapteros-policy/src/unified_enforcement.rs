...
impl UnifiedPolicyEnforcer {
    ...
    /// Add a policy pack
    pub fn add_policy_pack(&mut self, name: String, policy_pack: Box<dyn PolicyPack + Send + Sync>) {
...
impl PolicyEnforcer for UnifiedPolicyEnforcer {
    ...
    async fn is_operation_allowed(&self, operation: &Operation) -> Result<bool> {
        let validation_result = self.validate_request(&PolicyRequest {
    ...
    async fn get_compliance_report(&self) -> Result<PolicyComplianceReport> {
        ...
        for (pack_name, _policy_pack) in &self.policy_packs {
            let violations: Vec<_> = self.violation_history
                .iter()
            ...
            policy_pack_compliance.insert(pack_name.clone(), PolicyPackCompliance {
                policy_pack: pack_name.clone(),
                compliance_score,
                violation_count,
