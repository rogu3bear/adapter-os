...
fn draw_system_status(f: &mut Frame, app: &App, area: Rect) {
    ...
    let running = app.services.iter().filter(|s| s.status == Status::Running).count();
    let stopped = app.services.iter().filter(|s| s.status == Status::Stopped).count();
    let failed = app.services.iter().filter(|s| s.status == Status::Failed).count();

    let status_lines = vec![
        Line::from(vec![
            Span::styled("[OK] ", Style::default().fg(Color::Green)),
            Span::raw("Database        │ "),
            Span::styled(format!("{:<12}", "Connected"), Style::default().fg(Color::Green)),
            Span::raw(" │ Latency: 1.2ms"),
        ]),
        Line::from(vec![
            Span::styled("[OK] ", Style::default().fg(Color::Green)),
            Span::raw("Router          │ "),
            Span::styled(format!("{:<12}", "Ready"), Style::default().fg(Color::Green)),
            Span::raw(format!(" │ Adapters: {}/{}",
                app.metrics.active_adapters,
                app.metrics.total_adapters)),
        ]),
        Line::from(vec![
            Span::styled(
                if app.production_mode { "[OK] " } else { "[!!] " },
                Style::default().fg(if app.production_mode { Color::Green } else { Color::Yellow })
            ),
            Span::raw("Security        │ "),
            Span::styled(
                format!("{:<12}", if app.production_mode { "PRODUCTION" } else { "DEVELOPMENT" }),
                Style::default().fg(if app.production_mode { Color::Green } else { Color::Yellow })
            ),
            Span::raw(" │ "),
            Span::styled(
                if app.production_mode { "All policies enforced" } else { "Relaxed policies" },
        ...
        Line::from(vec![
            ...
            Span::styled(
                ...
                Style::default().fg(
                    if app.metrics.memory_headroom_percent >= 15.0 {
                        Color::Green
                    } else {
                        Color::Yellow
                    }
        ...
        Line::from(vec![
            ...
            Span::styled(
                if app.db_stats.database_connected { "Connected " } else { "Offline " },
                Style::default().fg(if app.db_stats.database_connected { Color::Green } else { Color::Red })
            ),
            Span::raw("│ "),
            ...
            Span::styled(
                ...
                Style::default().fg(if app.db_stats.active_training_jobs > 0 { Color::Cyan } else { Color::Gray })
            ),
...
fn draw_main_menu(f: &mut Frame, app: &App, area: Rect) {
    let block = Block::default()
        ...
        .border_style(Style::default().fg(
            if app.current_mode == Mode::Normal {
                Color::Green
            } else {
                Color::Gray
            }
