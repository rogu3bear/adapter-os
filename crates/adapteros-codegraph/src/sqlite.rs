...
use crate::{CallGraph, CodeGraph, SymbolId, SymbolNode, SymbolKind, TypeAnnotation, Visibility, Span};
...
impl CodeGraphDb {
    ...
    pub async fn save_codegraph(&self, codegraph: &CodeGraph) -> Result<()> {
        ...
        for (id, symbol) in &codegraph.symbols {
            let type_annotation_json = symbol.type_annotation
                .as_ref()
            ...
            sqlx::query(r#"
                INSERT INTO symbols (
                    id, name, kind, type_annotation, signature, docstring,
    ...
    pub async fn load_codegraph(&self) -> Result<CodeGraph> {
        ...
        for row in symbol_rows {
            ...
            let mut symbol = SymbolNode::new(id.clone(), name, kind, span, file_path);
            
            // Set optional fields
            if let Some(type_annotation_json): Option<String> = row.get("type_annotation") {
                if let Ok(type_annotation) = serde_json::from_str::<TypeAnnotation>(&type_annotation_json) {
                    symbol.type_annotation = Some(type_annotation);
                }
            }
            
            if let Some(signature): Option<String> = row.get("signature") {
                symbol.signature = Some(signature);
            }
            
            if let Some(docstring): Option<String> = row.get("docstring") {
            ...
            if let Some(module_path_json): Option<String> = row.get("module_path") {
    ...
    fn parse_symbol_kind(&self, kind_str: &str) -> Result<SymbolKind> {
        match kind_str {
            ...
            _ => Err(AosError::Database(format!("Unknown symbol kind: {}", kind_str))),
        }
    }
    ...
    fn parse_visibility(&self, visibility_str: &str) -> Result<Visibility> {
        match visibility_str {
            ...
            _ if visibility_str.starts_with("pub(in ") => {
                let path = visibility_str.strip_prefix("pub(in ")
                    .and_then(|s| s.strip_suffix(")"))
                    .unwrap_or("");
                Ok(Visibility::InPath(path.to_string()))
            }
            _ => Err(AosError::Database(format!("Unknown visibility: {}", visibility_str))),
        }
    }
