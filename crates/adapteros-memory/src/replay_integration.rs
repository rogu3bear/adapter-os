...
use crate::{MemoryMigrationEvent, MemoryLayoutHash, Result, MemoryWatchdogError};
use adapteros_core::B3Hash;
use adapteros_telemetry::replay::{ReplayEvent, ReplayBundle};
...
impl ReplayMemoryLogger {
    ...
    pub fn log_allocation(
        ...
        let event_counter = self.event_counter.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
    ...
    pub fn log_memory_pressure(
        ...
        warn!(
            ...
            pressure_level, used_memory, total_memory, 
            (used_memory as f32 / total_memory as f32) * 100.0, event_counter
        );
    ...
    pub fn to_replay_events(&self) -> Vec<ReplayEvent> {
        ...
        events.iter().map(|event| ReplayEvent {
            event_type: format!("memory.{}", format!("{:?}", event.event_type).to_lowercase()),
    ...
    pub fn create_replay_bundle(&self, cpid: String, plan_id: String, seed_global: B3Hash) -> ReplayBundle {
    ...
    pub fn clear(&self) {
        ...
        self.event_counter.store(1, std::sync::atomic::Ordering::SeqCst);
    }

    /// Enable or disable logging
    pub fn set_logging_enabled(&mut self, enabled: bool) {
        self.logging_enabled = enabled;
        info!("Memory replay logging {}", if enabled { "enabled" } else { "disabled" });
    }
    ...
    pub fn set_sampling_rate(&mut self, rate: f32) {
        ...
        info!("Memory event sampling rate set to {:.2}", self.sampling_rate);
    }
...
mod tests {
    ...
    fn test_memory_deallocation_logging() {
        ...
        let events = logger.get_memory_events();
        assert_eq!(events.len(), 1);
        assert!(matches!(events[0].event_type, MemoryEventType::Deallocation));
    }
    ...
    fn test_page_migration_logging() {
        ...
        let events = logger.get_memory_events();
        assert_eq!(events.len(), 1);
        assert!(matches!(events[0].event_type, MemoryEventType::PageMigration));
    }
    ...
    fn test_buffer_relocation_logging() {
        ...
        let events = logger.get_memory_events();
        assert_eq!(events.len(), 1);
        assert!(matches!(events[0].event_type, MemoryEventType::BufferRelocation));
    }
    ...
    fn test_memory_pressure_logging() {
        ...
        let events = logger.get_memory_events();
        assert_eq!(events.len(), 1);
        assert!(matches!(events[0].event_type, MemoryEventType::MemoryPressure));
    }
