...
use crate::writer::write_trace_bundle;
    ...
    pub fn read_all_events(self) -> Result<Vec<Event>> {
        ...
        for (line_no, line) in self.reader.lines().enumerate() {
            ...
            let event: Event = serde_json::from_str(&line)
                .map_err(|e| AosError::Telemetry(format!("Failed to parse event at line {}: {}", line_no, e)))?;
        ...
        match self.reader.read_line(&mut line) {
            ...
            Ok(_) => {
                if line.trim().is_empty() {
                    ...
                    let event: Event = serde_json::from_str(&line)
                        .map_err(|e| AosError::Telemetry(format!("Failed to parse event: {}", e)))?;
                    Ok(Some(event))
                }
            }
...
pub fn read_trace_bundle<P: AsRef<Path>>(path: P) -> Result<TraceBundle> {
    ...
    if events.is_empty() {
        return Err(AosError::Telemetry("No events found in trace file".to_string()));
    }
