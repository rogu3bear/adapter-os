...
pub fn IconRefresh(
    ...
    let has_label = !aria_label.is_empty();
    view! {
        <svg class=class xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden=move || (!has_label).then_some("true") role=move || has_label.then_some("img") aria-label=move || has_label.then(|| aria_label.clone())>
