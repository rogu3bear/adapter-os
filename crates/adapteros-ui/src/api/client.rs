...
impl ApiClient {
    ...
    /// Create a training dataset from a single uploaded document (async; returns job id).
    ///
    /// POSTs to `/v1/training/datasets/from-upload/async`.
    #[cfg(target_arch = "wasm32")]
    pub async fn create_training_dataset_from_upload_async(
        ...
        let resp_value = JsFuture::from(window.fetch_with_request(&request))
            .await
            .map_err(|_| ApiError::Network("Training dataset async request failed".into()))?;
        let resp: web_sys::Response = resp_value
            .dyn_into()
            .map_err(|_| ApiError::Network("Failed to cast Response".into()))?;
    ...
    /// Fetch a job detail record (used for polling async job completion).
