...
fn resolve_symlink_chain(path: impl AsRef<Path>) -> Result<Vec<PathBuf>> {
    ...
    for _ in 0..10 {
        ...
        if visited.contains(&current_path) {
            return Err(AosError::Io("Circular symlink detected".to_string()));
        }
...
pub fn create_safe_symlink(target: impl AsRef<Path>, link: impl AsRef<Path>) -> Result<()> {
    ...
    std::os::unix::fs::symlink(target, link)
        .map_err(|e| AosError::Io(format!("Failed to create symlink: {}", e)))?;
...
pub fn remove_safe_symlink(path: impl AsRef<Path>) -> Result<()> {
    ...
    if !path.is_symlink() {
        return Err(AosError::Io("Path is not a symlink".to_string()));
    }

    std::fs::remove_file(path)
        .map_err(|e| AosError::Io(format!("Failed to remove symlink: {}", e)))?;
