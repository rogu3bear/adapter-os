//! MLX backend implementation for FusedKernels trait

use crate::{LoRAAdapter, MLXModel};
...
impl MLXBackend {
    /// Create new MLX backend with loaded model
    pub fn new(model: MLXModel) -> Self {
        Self {
            model: Arc::new(model),
            adapters: Arc::new(RwLock::new(HashMap::new())),
            device: "MLX (Apple Silicon)".to_string(),
        }
    }
...
impl FusedKernels for MLXBackend {
    fn load(&mut self, _plan_bytes: &[u8]) -> Result<()> {
        // Plan loading not needed for MLX - model already loaded
        tracing::info!("MLX backend ready with {} adapters", self.adapter_count());
        Ok(())
    }
    ...
    fn run_step(&mut self, ring: &RouterRing, io: &mut IoBuffers) -> Result<()> {
        ...
        tracing::debug!(
            "MLX step complete: position={}, active_adapters={}, logits_len={}",
            io.position,
            ring.indices.len(),
            logits.len()
        );
