...
impl MetricsCollector {
    ...
    pub fn snapshot(&self) -> adapteros_telemetry::metrics::MetricsSnapshot {
        ...
        adapteros_telemetry::metrics::MetricsSnapshot {
            timestamp_ms: now_ms,
            ..Default::default()
        }
    }
...
impl MetricsSeries {
    ...
    pub fn get_points(&self, start: Option<u64>, end: Option<u64>) -> Vec<MetricDataPoint> {
        self.points
            .iter()
            .filter(|p| {
                let after_start = start.is_none_or(|s| p.timestamp >= s);
                let before_end = end.is_none_or(|e| p.timestamp <= e);
...
mod tests {
    ...
    #[test]
    #[ignore = "Pending API updates - prometheus Registry metric_count method not available [tracking: STAB-IGN-0068]"]
