...
pub async fn log_tenant_access_attempt(
    ...
    let id = crate::id_generator::readable_id(adapteros_id::IdPrefix::Aud, "tenant-access");
...
pub async fn track_auth_attempt(
    ...
    let id = crate::id_generator::readable_id(adapteros_id::IdPrefix::Evt, "auth-attempt");
    let now = Utc::now();
...
pub async fn track_registration_attempt(db: &Db, ip_address: &str) -> Result<()> {
    ...
    let id = crate::id_generator::readable_id(adapteros_id::IdPrefix::Evt, "registration");
