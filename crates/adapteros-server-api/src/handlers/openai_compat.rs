...
fn estimate_tokens(s: &str) -> usize {
    (s.len() + CHARS_PER_TOKEN_ESTIMATE - 1) / CHARS_PER_TOKEN_ESTIMATE
}
...
pub async fn chat_completions(
    ...
    infer_req.model = req.model.clone();
    infer_req.max_tokens = req
        .max_tokens
        .or(req.max_completion_tokens)
        .map(|v| v as usize);
    infer_req.temperature = req.temperature;
    infer_req.top_p = req.top_p;
