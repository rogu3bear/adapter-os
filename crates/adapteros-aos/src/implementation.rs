...
pub fn open_aos<'a>(bytes: &'a [u8]) -> Result<AosFileView<'a>> {
    ...
    let to_usize = |value: u64, label: &str| -> Result<usize> {
        usize::try_from(value).map_err(|_| {
            AosError::Validation(format!(
                "Corrupted / needs retrain: {} exceeds platform limits",
                label
            ))
        })
    };

    let index_offset = to_usize(
        u64::from_le_bytes(bytes[8..16].try_into().unwrap()),
        "index_offset",
    )?;
    let index_size = to_usize(
        u64::from_le_bytes(bytes[16..24].try_into().unwrap()),
        "index_size",
    )?;
    let manifest_offset = to_usize(
        u64::from_le_bytes(bytes[24..32].try_into().unwrap()),
        "manifest_offset",
    )?;
    let manifest_size = to_usize(
