...
//!
//! Run with: cargo run --release --example perf_test -p adapteros-lora-router
...
fn main() {
    ...
    for (name, inference_us) in simulated_inference_times {
        let avg_routing_us = (routing_time.as_micros() as u64) / (iterations as u64);
        let total_time_us = inference_us + avg_routing_us;
        let overhead_pct = (avg_routing_us as f64 / total_time_us as f64) * 100.0;
        println!(
