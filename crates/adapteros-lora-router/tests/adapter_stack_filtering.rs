...
fn test_no_stack_selects_all_eligible() {
    ...
    let decision_no_stack = router_no_stack
        .route_with_adapter_info(&features, &priors, &adapters, &policy_mask)
        .expect("router decision");
    let decision_with_full_stack = router_with_full_stack
        .route_with_adapter_info(&features, &priors, &adapters, &policy_mask)
        .expect("router decision");
...
fn test_stack_activation_deactivation() {
    ...
    let decision1 = router
        .route_with_code_features(&code_features1, &adapters)
        .expect("router decision");
    ...
    let decision2 = router
        .route_with_code_features(&code_features2, &adapters)
        .expect("router decision");
    ...
    let decision3 = router
        .route_with_code_features(&code_features3, &adapters)
        .expect("router decision");
...
fn test_deterministic_across_language_changes() {
    ...
    let decision1_a = router1
        .route_with_code_features(&python_code_features, &adapters)
        .expect("router decision");
    let decision1_b = router1
        .route_with_code_features(&python_code_features, &adapters)
        .expect("router decision");

    // Rust router with Rust features
    let decision2_a = router2
        .route_with_code_features(&rust_code_features, &adapters)
        .expect("router decision");
    let decision2_b = router2
        .route_with_code_features(&rust_code_features, &adapters)
        .expect("router decision");
...
fn test_stack_with_duplicate_adapters() {
    ...
    let policy_mask = allow_all_mask(&adapters);
    let decision = router
        .route_with_adapter_info(&python_features(), &priors, &adapters, &policy_mask)
        .expect("router decision");
