...
fn test_migration_signature_format() {
    ...
    if signatures_file.exists() {
        let content = fs::read_to_string(signatures_file)
            .expect("Failed to read signatures file");
        ...
        // Should not be empty
        assert!(!content.trim().is_empty(), "Signatures file should not be empty");
    } else {
    ...
    if migrations_dir.exists() {
        ...
        for entry in entries.filter_map(|e| e.ok()) {
            ...
            if file_name.ends_with(".sql") {
                assert!(file_name.chars().next().unwrap_or(' ').is_numeric(),
                       "Migration file '{}' should start with a number", file_name);
            }
        }
    }
}
