...
impl PerformanceVerifier {
    ...
    pub async fn verify(&self, config: &crate::unified_validation::PerformanceConfig) -> Result<PerformanceResult> {
    ...
    /// Run throughput analysis
    async fn run_throughput_analysis(&self, targets: &HashMap<String, f64>) -> Result<ThroughputResults> {
    ...
    fn calculate_score(
        ...
        if config.enable_performance_testing {
            let failure_rate = benchmarks.failed_benchmarks as f64 / benchmarks.total_benchmarks as f64;
