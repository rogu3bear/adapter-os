...
impl SecurityVerifier {
    ...
    pub async fn verify(&self, config: &crate::unified_validation::SecurityConfig) -> Result<SecurityResult> {
        ...
        let compliance_results = if config.enable_container_scanning {
            self.run_compliance_checks(&["SOC2".to_string(), "ISO27001".to_string()]).await?
        } else {
            ComplianceResults {
    ...
    fn calculate_score(
        ...
        if config.enable_dependency_scanning {
            let vulnerable_ratio = dependencies.vulnerable_dependencies as f64 / dependencies.total_dependencies as f64;
            score -= vulnerable_ratio * 20.0;
            
            let outdated_ratio = dependencies.outdated_dependencies as f64 / dependencies.total_dependencies as f64;
    ...
    fn generate_recommendations(
        ...
        if vulnerabilities.critical > 0 {
            recommendations.push(format!("Fix {} critical vulnerabilities immediately", vulnerabilities.critical));
        }
        
        if vulnerabilities.high > 0 {
            recommendations.push(format!("Address {} high severity vulnerabilities", vulnerabilities.high));
        }
        
        if dependencies.vulnerable_dependencies > 0 {
            recommendations.push(format!("Update {} vulnerable dependencies", dependencies.vulnerable_dependencies));
        }
        
        if dependencies.outdated_dependencies > 0 {
            recommendations.push(format!("Update {} outdated dependencies", dependencies.outdated_dependencies));
        }
        
        if code_security.unsafe_code_count > 0 {
            recommendations.push(format!("Review {} unsafe code blocks", code_security.unsafe_code_count));
        }
