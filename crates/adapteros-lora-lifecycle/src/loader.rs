...
impl AdapterLoader {
    ...
    pub async fn load_adapter_async(
        ...
        if self.use_mmap && adapter_path.extension() == Some(OsStr::new("aos")) {
            if let Some(ref mmap_loader) = self.mmap_loader {
                let mmap_loader_clone = mmap_loader.clone();
                return self
                    .load_aos_mmap(adapter_id, adapter_name, &mmap_loader_clone)
                    .await;
            }
        }
    ...
    async fn load_aos_mmap(
        ...
        let load_options = adapteros_single_file_adapter::LoadOptions {
            skip_verification: false,
            skip_signature_check: false,
            use_mmap: true,
        };
        let mmap_adapter = {
            ...
            loader_guard.load(&path, &load_options)?
        };
