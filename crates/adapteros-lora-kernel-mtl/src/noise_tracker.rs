...
use adapteros_numerics::noise::{EpsilonStats, GlobalStabilityReport, Tensor, measure_error};
...
impl NoiseTracker {
    ...
    pub fn track_layer_error(
        ...
        if self.layer_stats.len() >= self.config.max_layers_per_step {
            debug!("Maximum layers per step reached, skipping noise tracking for layer: {}", layer_id);
            return Ok(());
        }

        let quantized_tensor = Tensor::new(
            quantized_output.to_vec(),
            vec![quantized_output.len()],
        );

        let epsilon_stats = if let Some(ref_output) = reference_output {
            // Compare against reference
            let reference_tensor = Tensor::new(
                ref_output.to_vec(),
                vec![ref_output.len()],
            );
        ...
        // Store statistics
        self.layer_stats.insert(layer_id.to_string(), epsilon_stats.clone());
...
mod tests {
    ...
    fn test_track_step() {
        ...
        tracker.track_layer_error("layer1", &quantized, Some(&reference)).unwrap();
        tracker.track_layer_error("layer2", &quantized, Some(&reference)).unwrap();
