...
pub enum TestAction {
    ...
    /// Make API call
    ApiCall { method: String, url: String, body: Option<serde_json::Value> },
    
    /// Database operation
    DatabaseOperation { operation: String, query: String, params: Vec<serde_json::Value> },
    
    /// File operation
    FileOperation { operation: String, path: String, content: Option<String> },
    
    /// Network operation
    NetworkOperation { operation: String, host: String, port: u16 },
...
impl TestingFramework for UnifiedTestingFramework {
    ...
    async fn run_test(&self, test_case: &TestCase) -> Result<TestResult> {
        ...
        if test_result.assertion_results.iter().any(|r| r.status == TestStatus::Failed) {
    ...
    async fn run_suite(&self, suite: &TestSuite) -> Result<TestSuiteResult> {
        ...
        let summary = TestSummary {
            ...
            passed_tests: test_results.iter().filter(|r| r.status == TestStatus::Passed).count() as u32,
            failed_tests: test_results.iter().filter(|r| r.status == TestStatus::Failed).count() as u32,
            skipped_tests: test_results.iter().filter(|r| r.status == TestStatus::Skipped).count() as u32,
            error_tests: test_results.iter().filter(|r| r.status == TestStatus::Error).count() as u32,
            timeout_tests: test_results.iter().filter(|r| r.status == TestStatus::Timeout).count() as u32,
            success_rate: if test_results.is_empty() { 0.0 } else {
                test_results.iter().filter(|r| r.status == TestStatus::Passed).count() as f64 / test_results.len() as f64
            },
            average_execution_time_ms: if test_results.is_empty() { 0.0 } else {
                test_results.iter().map(|r| r.execution_time_ms).sum::<u64>() as f64 / test_results.len() as f64
            },
        };
        
        let suite_result = TestSuiteResult {
            suite_id: suite.id.clone(),
            status: if summary.failed_tests > 0 { TestStatus::Failed } else { TestStatus::Passed },
