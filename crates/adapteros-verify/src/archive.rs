...
impl GoldenRunArchive {
    ...
    pub fn sign(&mut self, keypair: &Keypair) -> VerifyResult<()> {
        ...
        let serialized = serde_json::to_vec(&archive_for_signing)
    ...
    pub fn load<P: AsRef<Path>>(dir_path: P) -> VerifyResult<Self> {
        ...
        let manifest_json = fs::read_to_string(&manifest_path)
            .map_err(|_| VerifyError::ArchiveCorrupted {
        ...
        // Load epsilon stats
        let epsilon_path = dir_path.join("epsilon_stats.json");
        let epsilon_json = fs::read_to_string(&epsilon_path)
            .map_err(|_| VerifyError::ArchiveCorrupted {
        ...
        // Load bundle hash
        let hash_path = dir_path.join("bundle_hash.txt");
        let hash_str = fs::read_to_string(&hash_path)
            .map_err(|_| VerifyError::ArchiveCorrupted {
        ...
        Ok(Self {
            metadata,
            epsilon_stats,
            bundle_hash: B3Hash::from_hex(&bundle_hash)
                .map_err(|_| VerifyError::ArchiveCorrupted {
...
mod tests {
    ...
    fn create_test_archive() -> GoldenRunArchive {
        let metadata = GoldenRunMetadata {
            ...
            global_seed: B3Hash::from_hex("b3:1111111111111111111111111111111111111111111111111111111111111111").unwrap(),
        };
        ...
        let bundle_hash = B3Hash::from_hex("b3:2222222222222222222222222222222222222222222222222222222222222222").unwrap();
    ...
    fn test_archive_load_missing() {
        ...
        assert!(matches!(result.unwrap_err(), VerifyError::GoldenRunNotFound { .. }));
    }
}
