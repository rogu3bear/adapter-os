...
impl CrashJournal {
    ...
    pub fn record_crash(&self, panic_info: &PanicHookInfo) -> Result<()> {
        ...
        let (prev_hash, attestation, policy_hash) = {
            let prev = self.prev_bundle_hash.lock().unwrap_or_else(|poisoned| {
                tracing::warn!("prev_bundle_hash lock poisoned during crash, recovering");
                poisoned.into_inner()
            });
            let att = self.provider_attestation.lock().unwrap_or_else(|poisoned| {
                tracing::warn!("provider_attestation lock poisoned during crash, recovering");
                poisoned.into_inner()
            });
            let pol = self.policy_hash.lock().unwrap_or_else(|poisoned| {
