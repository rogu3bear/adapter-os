...
impl<'a> TempFileGuard<'a> {
    ...
    pub async fn move_to(self, destination: PathBuf) -> Result<()> {
        ...
        fs::rename(&self.path, &destination).await
            .map_err(|e| AosError::Io(format!("Failed to move temp file to {}: {}", destination.display(), e)))?;

        debug!("Moved temp file {} to {}", self.path.display(), destination.display());
    ...
    pub async fn copy_to(&self, destination: PathBuf) -> Result<()> {
        ...
        fs::copy(&self.path, &destination).await
            .map_err(|e| AosError::Io(format!("Failed to copy temp file to {}: {}", destination.display(), e)))?;

        debug!("Copied temp file {} to {}", self.path.display(), destination.display());
        Ok(())
    }
}
...
impl<'a> Drop for TempFileGuard<'a> {
    fn drop(&mut self) {
        ...
        // Remove from active files (this is a best-effort cleanup)
        let file_id = self.file_id.clone();
        let manager = self.manager;
        tokio::spawn(async move {
            if let Err(e) = manager.remove_temp_file(&file_id).await {
                tracing::warn!("Failed to remove temp file from active list: {}", e);
            }
        });
    }
}
...
impl<'a> TempDirGuard<'a> {
    ...
    pub async fn create_file(&self, name: &str) -> Result<TempFileGuard<'a>> {
        ...
        debug!("Created file {} in temp directory {}", name, self.path.display());
    ...
    pub async fn create_subdir(&self, name: &str) -> Result<TempDirGuard<'a>> {
        ...
        fs::create_dir_all(&subdir_path).await
            .map_err(|e| AosError::Io(format!("Failed to create subdirectory in temp directory: {}", e)))?;
        ...
        debug!("Created subdirectory {} in temp directory {}", name, self.path.display());
    ...
    pub async fn move_to(self, destination: PathBuf) -> Result<()> {
        ...
        // Move the directory
        fs::rename(&self.path, &destination).await
            .map_err(|e| AosError::Io(format!("Failed to move temp directory to {}: {}", destination.display(), e)))?;

        debug!("Moved temp directory {} to {}", self.path.display(), destination.display());
    ...
    pub async fn copy_to(&self, destination: PathBuf) -> Result<()> {
        ...
        self.copy_directory_recursive(&self.path, &destination).await?;

        debug!("Copied temp directory {} to {}", self.path.display(), destination.display());
        Ok(())
    }
    ...
    fn drop(&mut self) {
        ...
        if self.path.exists() {
            if let Err(e) = std::fs::remove_dir_all(&self.path) {
                tracing::warn!("Failed to remove temp directory {}: {}", self.path.display(), e);
            } else {
                debug!("Cleaned up temp directory: {}", self.path.display());
            }
        }

        // Remove from active directories (this is a best-effort cleanup)
        let dir_id = self.dir_id.clone();
        let manager = self.manager;
        tokio::spawn(async move {
            if let Err(e) = manager.remove_temp_dir(&dir_id).await {
                tracing::warn!("Failed to remove temp directory from active list: {}", e);
            }
        });
    }
}
