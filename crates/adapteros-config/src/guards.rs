...
impl ConfigGuards {
    ...
    pub fn initialize() -> Result<()> {
        GUARD_STATE.set(RwLock::new(GuardState::new()))
            .map_err(|_| AosError::Config(
                "Guard system already initialized".to_string()
            ))?;
        Ok(())
    }
    ...
    pub fn freeze() -> Result<()> {
        let guard_state = GUARD_STATE.get()
            .ok_or_else(|| AosError::Config(
                "Guard system not initialized".to_string()
            ))?;
        
        let mut state = guard_state.write()
            .map_err(|_| AosError::Config(
                "Failed to acquire guard state lock".to_string()
            ))?;
    ...
    pub fn record_violation(operation: &str, message: &str) -> Result<()> {
        ...
        tracing::error!("Configuration freeze violation: {} - {}", operation, message);
        Ok(())
    }
...
pub fn safe_env_var(key: &str) -> Result<Option<String>> {
    if ConfigGuards::is_frozen() {
        ConfigGuards::record_violation("std::env::var", "Environment variable access prohibited after freeze")?;
...
pub fn strict_env_var(key: &str) -> Result<String> {
    ...
    std::env::var(key).map_err(|e| AosError::Config(format!(
