...
mod tests {
    ...
    async fn test_contact_category_conversion() {
        assert_eq!(
            ContactCategory::from_str("adapter").expect("Test category parsing should succeed"),
            ContactCategory::Adapter
        );
        assert_eq!(
            ContactCategory::from_str("REPOSITORY").expect("Test category parsing should succeed"),
            ContactCategory::Repository
        );
        assert_eq!(
            ContactCategory::from_str("User").expect("Test category parsing should succeed"),
            ContactCategory::User
        );
        assert!(ContactCategory::from_str("invalid").is_err());
    }
}
