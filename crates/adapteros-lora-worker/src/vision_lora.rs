    ...
    fn test_load_from_safetensors() {
        ...
        let tensors = [
            (
                ...
                TensorView::new(safetensors::Dtype::F32, vec![2, 2], bytemuck::cast_slice(&weights)).unwrap(),
            ),
            (
                "vision_lora.bias".to_string(),
                TensorView::new(safetensors::Dtype::F32, vec![4], bytemuck::cast_slice(&bias)).unwrap(),
            ),
        ];
