...
use adapteros_db::{kv_metrics, DbFactory, DbStorageBackend, RuntimeSession};
use adapteros_deterministic_exec::{
    global_ledger::GlobalTickLedger, init_global_executor, EnforcementMode, ExecutorConfig,
};
use adapteros_lora_worker::memory::UmaPressureMonitor;
use adapteros_manifest::ManifestV3;
use adapteros_server::cli::{normalize_jwt_mode, Cli};
use adapteros_server::model_seeding::{download_priority_models, seed_models_from_cache_if_empty};
...
use adapteros_server::shutdown::{
    apply_background_task_degraded, shutdown_signal_with_drain, ShutdownCoordinator,
};
...
async fn main() -> Result<()> {
    ...
    // Spawn alert watcher if enabled
    {
        ...
        if cfg.alerting.enabled {
            ...
            match alerting::spawn_alert_watcher(db.clone(), api_alerting_config) {
                Ok(alert_handle) => {
                    background_tasks.record_spawned("Alert watcher", false);
                    shutdown_coordinator.set_alert_handle(alert_handle);
                }
                Err(e) => {
                    background_tasks.record_failed("Alert watcher", &e.to_string(), false);
                    return Err(anyhow::anyhow!("Failed to spawn alert watcher: {}", e));
                }
            }
        }
    }
    ...
    if production_mode {
        ...
        let degraded = apply_background_task_degraded(&boot_state, &background_tasks).await;
        if !degraded {
