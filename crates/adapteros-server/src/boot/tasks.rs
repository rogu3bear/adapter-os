...
pub struct BackgroundTaskSpawner {
    ...
    /// Shared tracker for reporting background task status
    task_tracker: Option<Arc<BackgroundTaskTracker>>,
}
...
impl BackgroundTaskSpawner {
    ...
    /// Attach a shared background task tracker for health/status reporting.
    pub fn with_task_tracker(mut self, tracker: Arc<BackgroundTaskTracker>) -> Self {
        ...
        F: Future<Output = ()> + Send + 'static,
    {
        match spawn_deterministic(name.to_string(), future) {
            ...
            Err(e) => {
                ...
                self.record_failed(name, &err.message, true);
        ...
        F: Future<Output = ()> + Send + 'static,
    {
        match spawn_deterministic(name.to_string(), future) {
            Ok(handle) => {
                self.shutdown_coordinator.register_task(handle);
                self.spawned_tasks.push(name.to_string());
                self.record_spawned(name, false);
                info!(task = %name, "Background task started");
                Ok(())
            }
            Err(e) => {
                ...
                self.failed_tasks.push(name.to_string());
                self.record_failed(name, &err.message, false);
    ...
    fn record_spawned(&self, name: &str, critical: bool) {
        if let Some(tracker) = &self.task_tracker {
            tracker.record_spawned(name, critical);
        }
    }

    fn record_failed(&self, name: &str, error: &str, critical: bool) {
        if let Some(tracker) = &self.task_tracker {
            tracker.record_failed(name, error, critical);
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn test_spawner_records_tracker_failure() {
        let coordinator = ShutdownCoordinator::new();
        let tracker = Arc::new(BackgroundTaskTracker::default());
        let mut spawner =
            BackgroundTaskSpawner::new(coordinator).with_task_tracker(Arc::clone(&tracker));

        let result = spawner.spawn_optional("Test optional task", async {}, "fallback");
        assert!(result.is_err());

        let snapshot = tracker.snapshot();
        assert!(snapshot.spawned.is_empty());
        assert_eq!(snapshot.failed.len(), 1);
        assert_eq!(snapshot.failed[0].name, "Test optional task");
        assert!(!snapshot.failed[0].critical);
    }
}
