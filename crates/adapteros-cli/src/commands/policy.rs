...
impl PolicyCommand {
    pub fn run(self) -> Result<()> {
        match self.command {
            PolicySubcommand::List { implemented, format } => {
                list_policy_packs(implemented, format)
            }
            PolicySubcommand::Explain { policy } => explain_policy_pack(&policy),
            PolicySubcommand::Enforce { pack, all, dry_run } => {
...
fn explain_policy_pack(policy_ref: &str) -> Result<()> {
    ...
    let policy_id = if let Ok(id) = policy_ref.parse::<usize>() {
        ...
        list_policies()
            ...
            .map(|p| p.id)
            .ok_or_else(|| {
                adapteros_core::AosError::InvalidInput(format!(
                    "Policy '{}' not found",
                    policy_ref
                ))
            })?
