...
//!
//! Migrated from adapteros-server-api/src/handlers/models.rs for spoke pattern.
//! Provides model management operations including loading, unloading, validation,
//! and status queries.
...
use adapteros_server_api::model_status::aggregate_status;
...
pub async fn load_model(
    ...
    let request_id =
        adapteros_server_api::request_id::get_request_id().unwrap_or_else(|| "unknown".to_string());
...
pub async fn get_all_models_status(
    ...
    for (model_id, records) in grouped {
        ...
        model_responses.push(adapteros_server_api::types::BaseModelStatusResponse {
            model_id,
            model_name: model.name,
