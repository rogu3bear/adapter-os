...
fn test_error_accumulation_bounded() {
    ...
    assert!(total_error < 1.0, "Total error {} exceeds bound", total_error);
    ...
    assert!(final_report.stability_score() < 10.0, "Stability score {} too high", final_report.stability_score());
}
...
fn test_identical_runs_produce_identical_epsilon() {
    ...
    tracker1.track_layer_error("test_layer", &test_data, Some(&reference_data)).unwrap();
    tracker1.track_step().unwrap();

    // Second run with identical data
    let mut tracker2 = NoiseTracker::new(config, None);
    tracker2.track_layer_error("test_layer", &test_data, Some(&reference_data)).unwrap();
...
fn test_threshold_violation_detection() {
    ...
    let result = strict_tracker.track_layer_error(
        "test_layer",
        &quantized_data,
        Some(&reference_data),
    );

    assert!(result.is_err(), "Strict mode should fail on threshold violation");
    ...
    let result = warning_tracker.track_layer_error(
        "test_layer",
        &quantized_data,
        Some(&reference_data),
    );

    assert!(result.is_ok(), "Warning mode should not fail on threshold violation");
    
    // Verify threshold violation is recorded
    let report = warning_tracker.get_stability_report();
    assert!(!report.threshold_violations.is_empty(), "Threshold violation should be recorded");
}
...
fn test_telemetry_integration() {
    ...
    tracker.track_layer_error("test_layer", &quantized_data, Some(&reference_data)).unwrap();
    tracker.track_step().unwrap();
    ...
    assert!(content.contains("kernel.noise"), "Should contain kernel noise event");
    assert!(content.contains("kernel.step"), "Should contain kernel step event");
