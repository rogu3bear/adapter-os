...
async fn test_worker_crash_during_hotswap() {
    ...
    local
        .run_until(async {
            ...
            // Wait for crash
            let mut spins = 0;
            while !worker_state.is_crashed() && spins < 30 {
                yield_now().await;
                spins += 1;
            }
