...
fn bench_kv_cache_allocation() {
    ...
    println!("  Allocations: {} in {:?} ({:.2} µs/alloc)", 
             iterations, alloc_duration, alloc_duration.as_micros() as f64 / iterations as f64);
    println!("  Deallocations: {} in {:?} ({:.2} µs/free)", 
             iterations, free_duration, free_duration.as_micros() as f64 / iterations as f64);
