// Simple test that only depends on adapteros-config crate
// This bypasses adapteros-server-api compilation issues
...
use adapteros_config::{
    init_config, get_config, is_frozen,
    types::{PrecedenceLevel, ConfigValidationError},
    precedence::DeterministicConfig,
    loader::ConfigLoader,
    guards::{set_config_frozen, is_config_frozen, get_env_var, ConfigGuards}
};
...
fn test_config_get_or_default() {
    ...
    assert_eq!(config.get_or_default("non.existing.key", "DefaultValue"), "DefaultValue");
}
