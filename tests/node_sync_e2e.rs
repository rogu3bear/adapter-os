...
fn test_create_replication_manifest() {
    ...
    let adapter_ids = vec!["adapter1".to_string(), "adapter2".to_string(), "adapter3".to_string()];
...
fn test_chunk_descriptors() {
    ...
    for artifact in &manifest.artifacts {
        ...
        assert_eq!(total_chunk_size, artifact.size_bytes, "Chunk sizes should sum to artifact size");
    }
}
...
fn test_replication_session_id_uniqueness() {
    ...
    let manifest1 = create_manifest(&cas_store, &adapter_ids, uuid::Uuid::new_v4().to_string()).unwrap();
    let manifest2 = create_manifest(&cas_store, &adapter_ids, uuid::Uuid::new_v4().to_string()).unwrap();
    
    assert_ne!(manifest1.session_id, manifest2.session_id, "Session IDs should be unique");
}
...
fn test_manifest_signature_present() {
    ...
    assert!(!manifest.signature.is_empty(), "Manifest should have a signature");
