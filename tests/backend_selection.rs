...
fn test_metal_backend_unavailable_on_non_macos() {
    let result = create_backend(BackendChoice::Metal);
    assert!(result.is_err(), "Metal backend should not be available on non-macOS");
}
...
fn test_mlx_backend_requires_feature_flag() {
    ...
    assert!(result.is_err(), "MLX backend should require experimental-backends feature");
    let err_msg = format!("{:?}", result.unwrap_err());
    assert!(err_msg.contains("experimental-backends") || err_msg.contains("PolicyViolation"),
...
fn test_coreml_backend_requires_feature_flag() {
    ...
    assert!(result.is_err(), "CoreML backend should require experimental-backends feature");
    let err_msg = format!("{:?}", result.unwrap_err());
    assert!(err_msg.contains("experimental-backends") || err_msg.contains("PolicyViolation"),
            "Error should mention feature requirement or policy violation");
}
...
fn test_backend_attestation_validation() {
    ...
    assert!(report.validate().is_ok(), "Attestation report should validate");
    
    // Check key properties
    assert!(report.deterministic, "Backend should be deterministic");
    assert!(report.metallib_hash.is_some(), "Metal backend should provide metallib hash");
}
...
fn test_experimental_build_includes_all_backends() {
    ...
    if let Err(e) = mlx_result {
        ...
        assert!(!err_msg.contains("requires --features"),
