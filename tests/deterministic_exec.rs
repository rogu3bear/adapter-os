...
async fn test_deterministic_event_sequence() {
    ...
    for hash in &event_hashes[1..] {
        assert_eq!(
            *hash, first_hash,
            "Event sequences must be identical across runs"
        );
    }
}
    ...
    for sequence in &random_values[1..] {
        assert_eq!(
            *sequence, *first_sequence,
            "Random sequences must be identical"
        );
    }
}
...
async fn test_deterministic_task_order() {
    ...
    for _ in 0..5 {
        ...
        for i in 0..3 {
            ...
            executor
                .spawn_deterministic(format!("Ordered Task {}", i), async move {
...
async fn test_deterministic_timeout_events() {
    ...
    for count in &timeout_events[1..] {
        assert_eq!(
            *count, first_count,
            "Timeout event counts must be identical"
        );
    }
}
...
async fn test_identical_output_hashes_100_runs() {
    ...
    for run in 0..100 {
        ...
        for i in 0..10 {
            ...
            executor
                .spawn_deterministic(format!("Complex Task {}", i), async move {
    ...
    for (i, hash) in output_hashes.iter().enumerate() {
        assert_eq!(
            *hash, first_hash,
            "Output hash {} differs from first hash",
            i
        );
    }

    println!(
        "All 100 runs produced identical output hashes: {:?}",
        first_hash
    );
}
