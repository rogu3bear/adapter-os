...
async fn test_circuit_breaker_integration_with_inference() {
    ...
    assert!(matches!(
        breaker.state(),
        adapteros_core::CircuitState::Closed
    ));
    ...
    for _ in 0..3 {
        let result: Result<(), _> = breaker
            .call(async {
                Err(adapteros_core::AosError::Unavailable(
    ...
    let result = breaker.call(async { Ok("should be rejected") }).await;
    ...
    for _ in 0..2 {
        let result = breaker.call(async { Ok("recovery_success") }).await;
...
async fn test_circuit_breaker_metrics() {
    ...
    for _ in 0..2 {
        let _: Result<(), _> = breaker
            .call(async { Err(adapteros_core::AosError::Unavailable("fail".to_string())) })
            .await;
    }
