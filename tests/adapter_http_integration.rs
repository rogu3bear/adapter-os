...
async fn test_adapter_load_conflict_detection() {
    ...
    config
        .register_adapter("test-adapter", "Test Adapter")
        .await
        .unwrap();
    ...
    let result1 = config
        .operation_tracker
        .start_adapter_operation("test-adapter", "test-tenant", AdapterOperationType::Load)
        .await;

    assert!(
        result1.is_ok(),
        "First load operation should start successfully"
    );

    // Verify operation is tracked
    let operation = config
        .operation_tracker
        .is_adapter_operation_running("test-adapter", "test-tenant")
        .await;
    assert!(operation.is_some());
    assert!(matches!(
        operation.unwrap().operation_type,
        adapteros_server_api::operation_tracker::OperationType::Adapter(AdapterOperationType::Load)
    ));
    ...
    assert!(
        result3.is_ok(),
        "Load operation should work after completion"
    );
...
async fn test_adapter_unload_conflict_detection() {
    ...
    let result = config
        .operation_tracker
        .start_adapter_operation("test-adapter", "test-tenant", AdapterOperationType::Unload)
        .await;

    assert!(
        result.is_ok(),
        "Unload operation should start on cold adapter"
    );
...
async fn test_adapter_load_unload_cycle() {
    ...
    config
        .db
        .update_adapter_state("test-adapter", "warm", "loaded")
        .await
        .unwrap();

    // Now unload should work
    let unload_result = config
        .operation_tracker
        .start_adapter_operation("test-adapter", "test-tenant", AdapterOperationType::Unload)
        .await;
    ...
    assert!(
        unload_result.is_ok(),
        "Unload operation should start on warm adapter"
    );
    ...
    config
        .db
        .update_adapter_state("test-adapter", "cold", "unloaded")
        .await
        .unwrap();
    ...
    assert!(
        load_result.is_ok(),
        "Load operation should work on cold adapter"
    );
...
async fn test_adapter_operation_tracker_conflicts() {
    ...
    assert!(
        ...
        "Unload should conflict with ongoing load"
    );
    ...
    // Now unload should work
    let unload_result2 = config
        .operation_tracker
        .start_adapter_operation("test-adapter", "test-tenant", AdapterOperationType::Unload)
        .await;

    assert!(
        unload_result2.is_ok(),
        "Unload should work after load completion"
    );
...
async fn test_adapter_operation_cleanup_on_error() {
    ...
    let operation_after = config
        .operation_tracker
        .is_adapter_operation_running("test-adapter", "test-tenant")
        .await;
    assert!(
        operation_after.is_none(),
...
async fn test_adapter_concurrent_operations_different_adapters() {
    ...
    config
        .register_adapter("adapter1", "Adapter 1")
        .await
        .unwrap();
    config
        .register_adapter("adapter2", "Adapter 2")
        .await
        .unwrap();
    ...
    let result1 = config
        .operation_tracker
        .start_adapter_operation("adapter1", "test-tenant", AdapterOperationType::Load)
        .await;

    let result2 = config
        .operation_tracker
        .start_adapter_operation("adapter2", "test-tenant", AdapterOperationType::Load)
        .await;

    assert!(result1.is_ok(), "First adapter operation should start");
    assert!(
        result2.is_ok(),
        "Second adapter operation should start independently"
    );

    // Both should be tracked
    let op1 = config
        .operation_tracker
        .is_adapter_operation_running("adapter1", "test-tenant")
        .await;
    let op2 = config
        .operation_tracker
        .is_adapter_operation_running("adapter2", "test-tenant")
        .await;
    ...
    config
        .operation_tracker
        .complete_adapter_operation("adapter1", "test-tenant", AdapterOperationType::Load, true)
        .await;
    config
        .operation_tracker
        .complete_adapter_operation("adapter2", "test-tenant", AdapterOperationType::Load, true)
        .await;
