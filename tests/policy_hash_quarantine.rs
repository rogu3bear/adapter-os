...
async fn test_e2e_hash_mismatch_detection() {
    ...
    // Create watcher
    let watcher =
        PolicyHashWatcher::new(db.clone(), telemetry.clone(), Some("test-cp".to_string()));
...
async fn test_e2e_quarantine_enforcement() {
    ...
    watcher
        .register_baseline("Policy1", &baseline, None)
        .await
        .unwrap();

    let mutated = B3Hash::hash(b"config v2");
    watcher
        .validate_policy_pack("Policy1", &mutated)
        .await
        .unwrap();
...
async fn test_e2e_database_persistence() {
    ...
    // First session: register baseline
    {
        ...
        watcher
            .register_baseline("PersistentPolicy", &hash, None)
            .await
            .unwrap();
    }
...
async fn test_e2e_telemetry_logging() {
    ...
    watcher
        .register_baseline("LogPolicy", &baseline, None)
        .await
        .unwrap();

    // Valid hash - should log "valid" event
    watcher
        .validate_policy_pack("LogPolicy", &baseline)
        .await
        .unwrap();
    ...
    watcher
        .validate_policy_pack("LogPolicy", &mutated)
        .await
        .unwrap();
    ...
    watcher
        .validate_policy_pack("UnknownPolicy", &new_hash)
        .await
        .unwrap();
...
async fn test_e2e_multiple_violations() {
    ...
    watcher
        ...
        .unwrap();
    watcher
        .register_baseline("Policy3", &hash3, None)
        .await
        .unwrap();
    ...
    watcher
        .validate_policy_pack("Policy1", &mut1)
        .await
        .unwrap();
    watcher
        .validate_policy_pack("Policy2", &hash2)
        .await
        .unwrap(); // Still valid
    watcher
        .validate_policy_pack("Policy3", &mut3)
        .await
        .unwrap();
