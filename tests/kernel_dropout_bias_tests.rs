...
fn test_lora_dropout_config() {
    ...
    assert_eq!(config.dropout_rate, 0.0, "Default dropout should be 0.0 for inference");
...
fn test_deterministic_dropout_simulation() {
    ...
    for position in 0..100 {
        ...
        assert!(
            mask >= 0.0,
            "Dropout mask should be non-negative"
        );
...
fn test_attention_bias_integration() {
    ...
    assert_eq!(biases.k_bias.as_ref().unwrap().len(), num_kv_heads * head_dim);
    assert_eq!(biases.v_bias.as_ref().unwrap().len(), num_kv_heads * head_dim);
}
