...
async fn test_sql_injection_prevention_in_query_metrics() {
    ...
    use adapteros_db::process_monitoring::{MetricFilters, ProcessHealthMetric};
...
async fn test_sqlite_path_traversal_prevention() {
    ...
    for path in &malicious_paths {
        ...
        assert!(
            err_msg.contains("..") || err_msg.contains("path"),
...
async fn test_sqlite_sensitive_directory_prevention() {
    ...
    let sensitive_paths = ["/etc/passwd", "/root/.db", "/sys/db", "/proc/db", "/dev/db"];

    for path in &sensitive_paths {
        let result = Db::connect(path).await;
        assert!(
            result.is_err(),
            "Sensitive directory should be rejected: {}",
            path
        );
        ...
        assert!(
            err_msg.contains("sensitive") || err_msg.contains("directory"),
            "Error should mention sensitive directory: {}",
            err_msg
        );
    }
}
...
async fn test_postgres_network_validation_rejects_public_ip() {
    ...
    assert!(
        result.is_err(),
        "Public IP should be rejected in production"
    );
    ...
    assert!(
        err_msg.contains("public IP") || err_msg.contains("8.8.8.8"),
