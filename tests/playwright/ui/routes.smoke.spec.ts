...
async function runRouteChecks(page: Page, routes: RouteCheck[]): Promise<void> {
...
const routeGroups: Array<{ name: string; routes: RouteCheck[] }> = [
  {
    name: 'core',
    routes: [
...
for (const group of routeGroups) {
  test(`route smoke coverage (${group.name})`, async ({ page }) => {
    test.setTimeout(120_000);
    await runRouteChecks(page, group.routes);
  });
}
