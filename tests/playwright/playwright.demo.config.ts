...
const defaultDemoRunId = `demo-${new Date().toISOString().replace(/[-:.TZ]/g, '').slice(0, 14)}`;
const runId = sanitizeRunId(process.env.PW_RUN_ID ?? defaultDemoRunId);
...
export default defineConfig({
  ...
  use: {
    baseURL,
    // Always capture for demo recordings.
    trace: 'on',
    screenshot: 'on',
    video: 'on',
  ...
  projects: [
    // Demo recordings target chromium only for consistency.
    { name: 'chromium', use: { browserName: 'chromium' } },
  ],
  globalSetup: 'ui/global-setup.ts',
  globalTeardown: 'ui/global-teardown.ts',
  // Generous global timeout for full demo suites.
  globalTimeout: 30 * 60_000,
  // No retries — demo recordings should be deterministic.
  retries: 0,
  webServer: {
