...
use adapteros_policy::{MploraPolicy, MploraConfig as PolicyMploraConfig};
...
fn test_adapteros_policy_compliance() -> Result<()> {
    ...
    assert!(policy.check_orthogonal_compliance(&adapter_indices, &gates, &similarity_scores).is_ok());
    
    // High similarity violation
    let high_similarity_scores = vec![0.9, 0.8, 0.7];
    assert!(policy.check_orthogonal_compliance(&adapter_indices, &gates, &high_similarity_scores).is_err());
...
fn test_shared_downsample_determinism() -> Result<()> {
    ...
    assert_eq!(config1.orthogonal_constraints, config2.orthogonal_constraints);
    ...
    assert_eq!(deserialized1.shared_downsample, deserialized2.shared_downsample);
    assert_eq!(deserialized1.compression_ratio, deserialized2.compression_ratio);
...
fn test_mplora_integration() -> Result<()> {
    ...
    assert!(constraints.check_constraints(&decision.indices, &decision.gates_q15).is_ok());
