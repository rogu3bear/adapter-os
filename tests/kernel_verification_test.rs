...
mod tests {
    ...
    fn test_metal_kernel_compilation_and_deterministic_hash() {
        ...
        assert!(output.status.success(), "Metal build script failed: {}\nStderr: {}", 
        ...
        let kernel_hash_path = original_dir.join("crates/adapteros-lora-kernel-mtl/shaders/kernel_hash.txt");
        let compiled_hash = std::fs::read_to_string(&kernel_hash_path)
            .expect(&format!("Failed to read kernel_hash.txt from {:?}", kernel_hash_path))
            .trim()
            .to_string();
        ...
        assert!(second_output.status.success(), "Second Metal build script failed");
        ...
        assert_eq!(compiled_hash, second_hash, "Kernel hash is not deterministic across builds");
    ...
    fn test_kernel_registry_structure() {
        ...
        let registry_content = std::fs::read_to_string(&registry_path)
            .expect("Failed to read kernels.json");

        // Parse JSON to verify structure
        let registry: serde_json::Value = serde_json::from_str(&registry_content)
            .expect("Failed to parse kernels.json");

        // Verify required fields
        assert!(registry["schema_version"].is_string(), "Missing schema_version");
        assert!(registry["build_timestamp"].is_string(), "Missing build_timestamp");
        assert!(registry["metal_sdk_version"].is_string(), "Missing metal_sdk_version");
        assert!(registry["compiler_version"].is_string(), "Missing compiler_version");

        // Verify kernels array
        let kernels = registry["kernels"].as_array()
        ...
        for kernel in kernels {
            ...
            assert!(kernel["description"].is_string(), "Kernel missing description");
            assert!(kernel["parameters"].is_string(), "Kernel missing parameters");
            assert!(kernel["blake3_hash"].is_string(), "Kernel missing blake3_hash");
        ...
        let param_structures = registry["parameter_structures"].as_array()
        ...
        let config_structures = registry["configuration_structures"].as_array()
            .expect("configuration_structures field is not an array");
        
        assert_eq!(config_structures.len(), 3, "Expected 3 configuration structures");
