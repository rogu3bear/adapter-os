...
fn test_metal_kernel_compilation_and_hash() {
    ...
    assert!(output.status.success(), "Metal build script failed: {}", str::from_utf8(&output.stderr).unwrap_or("Invalid UTF-8"));
    ...
    assert!(std::path::Path::new(metallib_path).exists(), "Metallib file does not exist");
    ...
    let kernel_names: Vec<&str> = kernels.iter()
        .map(|k| k["name"].as_str().unwrap())
        .collect();
    
    assert!(kernel_names.contains(&"fused_mlp"), "Missing fused_mlp kernel");
    assert!(kernel_names.contains(&"fused_qkv_gqa"), "Missing fused_qkv_gqa kernel");
    assert!(kernel_names.contains(&"flash_attention"), "Missing flash_attention kernel");
    
    // Verify each kernel has the expected hash
    for kernel in kernels {
        let kernel_hash = kernel["blake3_hash"].as_str().unwrap();
        assert_eq!(kernel_hash, compiled_hash, "Kernel hash mismatch for {}", kernel["name"].as_str().unwrap());
    }
}
...
fn test_deterministic_kernel_hashes() {
    ...
    assert_eq!(hash1, hash2, "Kernel hashes are not deterministic: {} != {}", hash1, hash2);
