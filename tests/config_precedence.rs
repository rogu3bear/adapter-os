//! Configuration precedence and freeze enforcement tests

use adapteros_config::{
    ConfigLoader, ConfigGuards, initialize_config, get_config, is_frozen,
...
"#).unwrap();
    ...
    assert_eq!(config.get("policy.strict_mode"), Some(&"true".to_string()));
    assert_eq!(config.get("policy.audit_logging"), Some(&"false".to_string()));
}
...
fn test_config_cli_boolean_flags() {
    let loader = ConfigLoader::new();
    let config = loader.load(
        vec!["--policy.strict_mode".to_string()],
        None,
    ).unwrap();
