...
{"event_type": "test2", "payload": {}, "timestamp": 1234567891}"#;
    fs::write(bundles_path.join(format!("{}_replay.ndjson", test_cpid)), replay_content)?;
...
seeds:
  ...
  manifest_hash: b3:manifest123"#;
    ...
    fs::write(manifests_path.join(format!("{}.yaml", test_cpid)), manifest_content)?;
...
async fn test_orchestrator_gate_failure() -> Result<()> {
    ...
    if let Some(metrics_result) = report.gates.get("Metrics") {
        assert!(!metrics_result.passed, "Metrics gate should fail with bad metrics");
        assert!(metrics_result.message.contains("Hallucination metrics failed"));
    }
