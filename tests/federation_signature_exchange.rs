...
async fn test_federation_signature_exchange() -> Result<()> {
    ...
    let federation_a = FederationManager::new(
        Arc::new(db_a),
        Arc::new(telemetry_a),
        "host-a".to_string(),
    )?;
    
    let federation_b = FederationManager::new(
        Arc::new(db_b),
        Arc::new(telemetry_b),
        "host-b".to_string(),
    )?;
    ...
    // Verify signatures
    let verification_a = federation_a.verify_signature(&signature_a, &bundle_metadata).await?;
    let verification_b = federation_b.verify_signature(&signature_b, &bundle_metadata).await?;
    
    assert!(verification_a.is_valid);
    assert!(verification_b.is_valid);
    
    // Test cross-host verification
    let cross_verification = federation_a.verify_signature(&signature_b, &bundle_metadata).await?;
    assert!(cross_verification.is_valid);
    
    // Test signature chain verification
    let chain_result = federation_a.verify_cross_host_chain(vec![signature_a, signature_b]).await?;
...
async fn test_federation_output_hash_comparison() -> Result<()> {
    ...
    federation_a.record_output_hash(session_id, &output_hash).await?;
    federation_b.record_output_hash(session_id, &output_hash).await?;
    ...
    match comparison {
        ...
        adapteros_federation::ComparisonResult::Mismatch(divergences) => {
            panic!("Output hashes should match, but found divergences: {:?}", divergences);
        }
    ...
    federation_b.record_output_hash("mismatch-session", &different_hash).await?;
    
    let mismatch_comparison = federation_a.compare_output_hashes("mismatch-session").await?;
