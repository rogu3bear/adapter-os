    ...
    for scale in scales {
        ...
        if scale < 1.0 {
            assert!(
                ...
                "Scaling should reduce large scores"
            );
        }
    }
}
...
fn test_scaling_impact_on_softmax() {
    ...
    assert!(
        (sum_no_scale - 1.0).abs() < 1e-5,
        "Softmax should sum to 1.0"
    );
    assert!(
        (sum_with_scale - 1.0).abs() < 1e-5,
        "Softmax should sum to 1.0"
    );
...
fn test_sqrt_scaling_formula() {
    ...
    for head_dim in head_dims {
        ...
        if head_dim > 32 {
            ...
            assert!(scale < prev_scale, "Scale should decrease with head_dim");
        }
...
fn test_attention_score_range() {
    ...
    for dot_prod in dot_products {
        ...
        assert!(
            scaled_score < 20.0,
            "Scaled score should be < 20 for numerical stability"
        );
        assert!(
            scaled_score > -20.0,
            "Scaled score should be > -20 for numerical stability"
