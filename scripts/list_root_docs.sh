...
for file in README.md CONTRIBUTING.md SECURITY.md LICENSE LICENSE-APACHE LICENSE-MIT CHANGELOG.md AGENTS.md CITATIONS.md QUICKSTART.md QUICKSTART_GPU_TRAINING.md PRD.md CODE_OF_CONDUCT.md; do
    if [ -f "$file" ]; then
        echo "  - $file"
    fi
done
...
find . -maxdepth 1 -name "*.md" -type f | sed 's|^\./||' | sort | while read -r file; do
    ...
    if [[ ! "$file" =~ ^(README|CONTRIBUTING|SECURITY|LICENSE|CHANGELOG|AGENTS|CITATIONS|QUICKSTART|PRD|CODE_OF_CONDUCT)\.md$ ]] && \
