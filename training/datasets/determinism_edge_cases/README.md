...
This dataset directly tests the patterns documented in `/home/user/adapter-os/CLAUDE.md`:

### Deterministic Executor Seeding
From `CLAUDE.md#deterministic-executor-seeding`:
```rust
...
### Global Tick Ledger
From `CLAUDE.md#global-tick-ledger-issue-c-6-fix`:
```rust
...
From `CLAUDE.md#multi-agent-coordination-dead-agent-handling-issue-c-8`:
```rust
